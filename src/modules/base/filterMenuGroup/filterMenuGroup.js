...
export default class FilterMenuGroup extends LightningElement {
     ...
     *
     * @type {AvonniFilterMenuGroupMenu[]}
     * @public
     */
    @api
    get menus() {
    ...
    get computedButtonIconPopoverClass() {
        const hasSelectedPopover = (this.hiddenMenus || []).some((menu) => {
            const val = this._value[menu.name];
    ...
    updateVisibleMenusOnResize() {
        ...
        const unscaledHeight = this.menuGroupWrapper?.offsetHeight;
        if (
            (this._containerMaxHeight === height ||
                this._containerMaxHeight === unscaledHeight) &&
