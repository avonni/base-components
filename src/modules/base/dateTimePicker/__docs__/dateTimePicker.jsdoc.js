/**
 * @typedef {Object} DateTimePickerAvatar
 * @name avatar
 * @property {string} fallbackIconName The Lightning Design System name of the icon used as a fallback when the option avatar image fails to load. Specify the name in the format 'utility:user' where 'utility' is the category, and 'user' is the specific icon to be displayed.
 * @property {string} initials If the record name contains two words, like first and last name, use the first capitalized letter of each. For records that only have a single word name, use the first two letters of that word using one capital and one lower case letter.
 * @property {string} size Size of the avatar. Valid values include x-small, small, medium, large, x-large and xx-large. Defaults to medium.
 * @property {string} src Image URL for the avatar.
 * @property {string} variant The variant changes the shape of the avatar. Accepted variants include circle and square. Defaults to square.
 */

/**
 * @namespace stylingHooks
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-color-background
 * @default #ffffff
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-color-background-disabled
 * @default #f3f2f2
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-color-background-selected
 * @default #d8edff
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-color-border
 * @default #dddbda
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-color-border-selected
 * @default #0070d2
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-sizing-border
 * @default 1px
 * @type dimension
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-sizing-border-selected
 * @default 1px
 * @type dimension
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-styling-border
 * @default solid
 * @type keyword
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-styling-border-selected
 * @default solid
 * @type keyword
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-text-color
 * @default #181818
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-text-color-selected
 * @default #181818
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-button-text-color-disabled
 * @default #706e6b
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-date-label-line-height
 * @default 1.25
 * @type number
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-date-label-font-size
 * @default 1.25rem
 * @type dimension
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-date-label-font-weight
 * @default 400
 * @type integer
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-date-picker-button-color-background-hover
 * @default #f3f3f3
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-date-picker-button-color-background-selected
 * @default #032d60
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-date-picker-button-text-color-hover
 * @default #181818
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-date-picker-button-text-color-selected
 * @default #ffffff
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-label-font-size
 * @default 0.75rem
 * @type dimension
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-label-font-style
 * @default normal
 * @type keyword
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-label-font-weight
 * @default 700
 * @type integer
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-label-horizontal-alignment
 * @default left
 * @type alignment
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-label-line-height
 * @default 1.5
 * @type number
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-label-text-color
 * @default #444444
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-weekly-day-header-text-color
 * @default #706e6b
 * @type color
 */
/**
 * @memberof stylingHooks
 * @name --avonni-date-time-picker-weekly-day-header-text-color-today
 * @default #1589ee
 * @type color
 */
